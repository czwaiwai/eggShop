...
const { txtMsg } = require('./wxMsgTpl'); // 回复的消息模板
...
function genSignature(jsapi_ticket, noncestr, timestamp, url) {
  const tmp = [ jsapi_ticket, noncestr, timestamp, url ].sort().join('&');
  return crypto.createHash('sha1').update(tmp).digest('hex');
}
let instance = null;
Wechat.getInstance = function() {
  if (!instance) {
    const config = require('./wx.json');
...
Wechat.prototype.setMenu = function(json) {
  return new Promise((resolve, reject) => {
    this.getAccessToken().then(res => {
      const url = util.format(this.config.apiURL.createMenu, this.config.appDomain, res);
...
Wechat.prototype.textHandle = function(req, res, next) {
  ...
  console.log(data, '------------------------');
  const formUser = data.tousername; // 开发者微信号
  const toUser = data.fromusername; // 用户的openId
...
Wechat.prototype.getAccessToken = function() {
  return new Promise((resolve, reject) => {
    ...
    const url = util.format(this.config.apiURL.accessTokenApi, this.config.appDomain, this.config.appID, this.config.appScrect);
    let accessToken = {};
...
Wechat.prototype.getSignTicket = function(ticket, url) {
  ...
  const signature = genSignature(`jsapi_ticket=${ticket}`, `noncestr=${noncestr}`, `timestamp=${timestamp}`, `url=${url}`);
  return {
...
Wechat.prototype.authLogin = function() {
  ...
  const reirect_uri = encodeURIComponent('http://www.bssfood.com/auth');
  const scope = 'snsapi_base';
  const state = 'bssfood';
  const reqUrl = `https://open.weixin.qq.com/connect/oauth2/authorize?appid=${appid}&redirect_uri=${reirect_uri}&response_type=code&scope=${scope}&state=${state}#wechat_redirect`;
  return new Promise((resolve, reject) => {
...
Wechat.prototype.getCodeToken = function(code) {
  ...
  // }
  const reqUrl = `https://api.weixin.qq.com/sns/oauth2/access_token?appid=${this.config.appID}&secret=${this.config.appScrect}&code=${code}&grant_type=authorization_code`;
  return new Promise((resolve, reject) => {
    request.get(reqUrl, (err, response, body) => {
      if (err) {
        return reject(err);
...
Wechat.prototype.getUserInfo = function(accToken, openId) {
  const reqUrl = `https://api.weixin.qq.com/sns/userinfo?access_token=${accToken}&openid=${openId}&lang=zh_CN`;
  return new Promise((resolve, reject) => {
    request.get(reqUrl, (err, response, body) => {
      if (err) {
        return reject(err);
...
Wechat.prototype.reflashToken = function() {
  // const reqUrl = 'https://api.weixin.qq.com/sns/oauth2/refresh_token?appid=APPID&grant_type=refresh_token&refresh_token=REFRESH_TOKEN';
  // { "access_token":"ACCESS_TOKEN",
  // 	"expires_in":7200,
  // 	"refresh_token":"REFRESH_TOKEN",
  // 	"openid":"OPENID",
